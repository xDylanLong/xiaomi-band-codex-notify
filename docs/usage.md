...
```bash
$ANDROID_SDK_ROOT/platform-tools/adb install -r android-companion/releases/小米手环Codex通知-v0.3.5.apk
```

如果手机中已有 v0.3.1 或更早版本，需要先卸载旧 App，再安装 v0.3.5；这是因为旧版本使用了不同签名。v0.3.2 及之后的版本会复用固定签名，可以直接覆盖升级。

也可以把 APK 传到 Android 手机后手动安装：

`android-companion/releases/小米手环Codex通知-v0.3.5.apk`
...
脚本使用 `javac`、`d8`、`aapt2`、`zipalign` 和 `apksigner`，不依赖 Gradle 下载。输出为 `android-companion/releases/小米手环Codex通知-v0.3.5.apk`。固定签名密钥默认保存在被 Git 忽略的 `android-companion/.signing/release.keystore`，请发布者自行备份，不要提交到公开仓库。
